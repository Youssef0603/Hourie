...
import { createEmployee, createSite, getEmployee, getEmployees, getSites, updateEmployee } from '../api'
...
export function SitesPage({ canAdd, onOpenSite, catalogs }: DirectoryPageProps) {
  ...
  const projectStatusLabels = fr.status.project as Record<string, string>
  catalogOptions(catalogs, 'project_status').forEach((option) => {
    projectStatusLabels[option.code] = catalogLabel(catalogs, 'project_status', option.code)
  })
  ...
  return (
    <main className="directory-page">
      ...
      {showForm && <Modal title={fr.directory.addSite} size="wide" onClose={() => setShowForm(false)}><form className="site-form" onSubmit={submit}>{error && <div className="form-alert" role="alert">{error}</div>}<div className="maintenance-form-grid"><label><span>{fr.directory.siteName}</span><input required value={name} onChange={(event) => setName(event.target.value)} /></label><label><span>{fr.directory.siteStatus}</span><select value={status} onChange={(event) => setStatus(event.target.value as ProjectStatus)}>{Object.entries(fr.status.project).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="field-wide"><span>{fr.directory.siteAddress}</span><input value={address} onChange={(event) => setAddress(event.target.value)} /></label><label><span>{fr.directory.startDate}</span><input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></label><label><span>{fr.directory.expectedEndDate}</span><input type="date" min={startDate || undefined} value={expectedEndDate} onChange={(event) => setExpectedEndDate(event.target.value)} /></label><div className="site-location-fields field-wide"><div className="site-location-fields-heading"><span>{fr.directory.initialLocations}</span><button type="button" onClick={() => setLocations((current) => [...current, ''])}><ActionIcon name="add" />{fr.directory.addLocation}</button></div>{locations.map((location, index) => <div className="site-location-input" key={index}><input required aria-label={`${fr.directory.locationName} ${index + 1}`} placeholder={fr.directory.locationName} value={location} onChange={(event) => updateLocation(index, event.target.value)} />{locations.length > 1 && <button type="button" onClick={() => removeLocation(index)} aria-label={fr.common.delete}><ActionIcon name="close" /></button>}</div>)}</div><label className="field-wide"><span>{fr.directory.siteNotes}</span><textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div><div className="maintenance-form-actions"><button type="button" onClick={() => setShowForm(false)}>{fr.common.cancel}</button><button className="primary-button" type="submit">{fr.common.save}</button></div></form></Modal>}
        ...
        {sites.map((site) => {
          ...
          return <button className="directory-card site-card" type="button" key={site.id} onClick={() => onOpenSite?.(site)}>
            <header className="site-card-header">
              ...
              <span className={`project-status project-status-${site.status}`} style={catalogBadgeStyle(catalogs, 'project_status', site.status)}>{(fr.status.project as Record<string, string>)[site.status] ?? site.status}</span>
            </header>
...
export function PeoplePage({ canAdd }: DirectoryPageProps) {
  ...
  return (
    <main className="directory-page">
      ...
      {(isLoadingPerson || selectedPerson) && <Modal title={isEditingPerson ? fr.directory.editPerson : selectedPerson?.name ?? fr.directory.personDetails} size="wide" onClose={closePerson}>{isLoadingPerson ? <LoadingSpinner className="person-detail-loading" label={fr.common.loading} /> : selectedPerson && (isEditingPerson ? <EditPersonForm person={selectedPerson} onCancel={() => setIsEditingPerson(false)} onSaved={(updated) => { setSelectedPerson(updated); setIsEditingPerson(false); refreshPeople() }} /> : <PersonDetail person={selectedPerson} canEdit={canAdd} onEdit={() => setIsEditingPerson(true)} />)}</Modal>}
    </main>
  )
}

function PersonDetail({ person, canEdit, onEdit }: { person: EmployeeDetails; canEdit: boolean; onEdit: () => void }) {
  return <div className="person-detail"><section><div className="person-detail-heading"><h3>{fr.directory.contactInformation}</h3>{canEdit && <button className="equipment-edit-button" type="button" onClick={onEdit}><ActionIcon name="edit" />{fr.common.edit}</button>}</div><dl><div><dt>{fr.directory.email}</dt><dd>{person.user?.email ?? fr.common.notAssigned}</dd></div><div><dt>{fr.directory.phoneNumber}</dt><dd>{person.phone_number ?? fr.common.notAssigned}</dd></div><div><dt>{fr.directory.role}</dt><dd>{person.user ? fr.roles[person.user.role] : fr.common.notAssigned}</dd></div><div><dt>{fr.directory.accountStatus}</dt><dd><span className={`account-status ${person.is_active ? 'active' : 'inactive'}`}>{person.is_active ? fr.directory.active : fr.directory.inactive}</span></dd></div></dl></section><section><div className="section-heading-row"><div><h3>{fr.directory.assignedEquipment}</h3><p>{fr.directory.custodyCount(person.equipment_in_custody_count)}</p></div></div>{person.equipment_in_custody.length > 0 ? <div className="site-inventory-wrap"><table className="equipment-table person-inventory-table"><thead><tr><th>{fr.equipment.number}</th><th>{fr.equipment.identification}</th><th>{fr.equipment.condition}</th><th>{fr.equipment.project}</th><th>{fr.equipment.locationShort}</th></tr></thead><tbody>{person.equipment_in_custody.map((equipment) => <tr key={equipment.id}><td><strong>{equipment.display_id}</strong><span className="secondary-cell">{equipment.asset_code}</span></td><td>{[equipment.brand, equipment.model].filter(Boolean).join(' ') || fr.common.notProvided}</td><td><span className={`status-badge status-${equipment.condition ?? 'unknown'}`}>{equipment.condition ? fr.status.condition[equipment.condition] : fr.common.notProvided}</span></td><td>{equipment.current_project_assignment?.project.name ?? fr.common.notProvided}</td><td>{equipment.current_location?.name ?? fr.common.notProvided}</td></tr>)}</tbody></table></div> : <p>{fr.directory.noGenerators}</p>}</section></div>
}

function EditPersonForm({ person, onCancel, onSaved }: { person: EmployeeDetails; onCancel: () => void; onSaved: (person: EmployeeDetails) => void }) {
  ...
  return <form className="directory-form people-form modal-directory-form person-edit-form" onSubmit={submit}><p className="account-form-hint">{fr.directory.editCredentialsHint}</p>{formError && <div className="form-alert account-form-alert" role="alert">{formError}</div>}<label><span>{fr.directory.fullName}</span><input required autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} /></label><label><span>{fr.directory.phoneNumber}</span><input type="tel" autoComplete="tel" value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} /></label><label><span>{fr.directory.loginEmail}</span><input required={person.user !== null} type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label><span>{fr.directory.accountRole}</span><select value={role} onChange={(event) => setRole(event.target.value as typeof role)}>{Object.entries(fr.roles).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>{fr.directory.newPassword}</span><input minLength={12} required={!person.user && email !== ''} type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><label><span>{fr.directory.confirmNewPassword}</span><input minLength={12} required={password !== ''} type="password" autoComplete="new-password" value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.target.value)} /></label><div className="maintenance-form-actions"><button type="button" onClick={onCancel}>{fr.common.cancel}</button><button className="primary-button" type="submit" disabled={isSaving}>{isSaving ? <LoadingSpinner compact label={fr.common.saving} /> : fr.common.save}</button></div></form>
}
