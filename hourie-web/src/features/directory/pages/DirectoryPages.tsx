...
export function PeoplePage({ canAdd, catalogs }: DirectoryPageProps) {
  ...
  return (
    <main className="directory-page">
      ...
      {(isLoadingPerson || selectedPerson) && <Modal title={isEditingPerson ? fr.directory.editPerson : selectedPerson?.name ?? fr.directory.personDetails} size="wide" onClose={closePerson}>{isLoadingPerson ? <LoadingSpinner className="person-detail-loading" label={fr.common.loading} /> : selectedPerson && (isEditingPerson ? <EditPersonForm person={selectedPerson} onCancel={() => setIsEditingPerson(false)} onSaved={(updated) => { setSelectedPerson(updated); setIsEditingPerson(false); refreshPeople() }} /> : <PersonDetail person={selectedPerson} catalogs={catalogs} canEdit={canAdd} onEdit={() => setIsEditingPerson(true)} onDelete={removePerson} />)}</Modal>}
    </main>
  )
}
